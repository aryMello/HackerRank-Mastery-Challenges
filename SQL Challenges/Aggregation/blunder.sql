...
for all employees in the EMPLOYEES table, but did not realize her keyboard's key 
...
*/
SELECT CAST(CEILING(AVG(CAST(SALARY AS FLOAT)) - AVG(CAST(REPLACE(SALARY, 0, '') AS FLOAT))) AS INTEGER) FROM EMPLOYEES;